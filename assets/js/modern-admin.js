...
(function() {
    ...
    function initSessionTimer() {
        ...
        if (typeof window.sessionRemainingTime !== 'undefined' && window.sessionRemainingTime > 0) {
            // Use server-side remaining time (most accurate)
            timeLeft = Math.max(0, window.sessionRemainingTime);
        } else if (typeof window.sessionLoginTime !== 'undefined' && window.sessionLoginTime > 0 && typeof window.sessionDuration !== 'undefined') {
            // Calculate from login time using server time
            const currentServerTime = Math.floor(Date.now() / 1000) + clientServerOffset;
        ...
        document.addEventListener('visibilitychange', function() {
            if (!document.hidden && typeof window.sessionRemainingTime !== 'undefined' && window.sessionRemainingTime >= 0) {
